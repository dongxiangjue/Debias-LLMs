...
def main():
    ...
    if args.tune_temp:
        for temp in temp_list:
            ...
            output_file = f"{output_dir}/corpus_generation_{args.dataset}_temp_{temp}"
    ...
    if args.tune_topp:
        for topp in topp_list:
            ...
            output_file = f"{output_dir}/corpus_generation_{args.dataset}_topp_{topp}"
            with open(f"{output_file}.json", "w") as f:
                json.dump(results, f)
                f.write('\n')
            torch.save(all_logits, f"{output_file}.pt")
    ...
    if args.tune_topk:
        for topk in topk_list:
            ...
            output_file = f"{output_dir}/corpus_generation_{args.dataset}_topk_{topk}"
            with open(f"{output_file}.json", "w") as f:
                json.dump(results, f)
                f.write('\n')
            torch.save(all_logits, f"{output_file}.pt")
    ...
    if args.tune_length:
        for length in length_list:
            ...
            output_file = f"{output_dir}/corpus_generation_{args.dataset}_length_{length}"
            with open(f"{output_file}.json", "w") as f:
                json.dump(results, f)
                f.write('\n')
            torch.save(all_logits, f"{output_file}.pt")
